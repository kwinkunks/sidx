...
:copyright: Ali Muldal
:source: http://stackoverflow.com/a/12254938/3381305
:license: Unknown
"""
import numpy as np
# import scipy as sp
...
# Otherwise use the normal scipy fftpack ones instead (~2-3x slower!)
    ...
    msg = "Module 'anfft' (FFTW Python bindings) could not be imported. To\n"
    msg += "install it, try running 'easy_install anfft' from the terminal.\n"
    msg += "Falling back on 'fftpack' module for ND Fourier transforms."
    print(msg)
...
class TemplateMatch(object):
    ...
    def __init__(self, template, method='ssd'):
        ...
        if method not in ['ncc', 'ssd', 'both']:
            raise Exception('Invalid method "%s". Valid methods are "ncc", "ssd" or "both"' % method)
...
def ndpad(a, npad=None, padval=0):
    ...
    *If npad is not a whole number, padding will be applied so that the
    ...
    if npad is None:
        ...
        msg = 'Length of npad (%i) does not match the dimensionality '
        msg += 'of the input array (%i)'
        raise Exception(msg % (len(npad), a.ndim))
    ...
    ind = [slice(np.floor(npad[dd]), a.shape[dd]+np.floor(npad[dd])) for dd in range(a.ndim)]
...
#               %(len(npad), b.ndim))
#   ind = [slice(np.floor(npad[dd]), b.shape[dd]-np.ceil(npad[dd])) for dd in range(b.ndim)]
#   return b[ind]
...
def procrustes(a, target, side='both', padval=0):
    ...
    try:
        if len(target) != a.ndim:
            msg = 'Target shape must have the same number of dimensions as the input'
            raise TypeError(msg)
    except TypeError:
